...
defmodule Ex04 do
  ...
  def even_or_odd(integer, {even, odd}) do 
    cond do
      Integer.is_even(integer) -> {[integer| even], odd}
      Integer.is_odd(integer) -> {even, [integer| odd]} 
    end
  end
  def even_odd(list) do
   {even, odd} = reduce(list, {[],[]}, &even_or_odd/2) 
